 ...
 * Without this, puts_scroll_worker(), scroll_stop_viewport_rect() and the
 * scroll worker thread race on the shared scroll[] array — observed on
 * SHANLING_M0PRO (touch-primary UI + DMA LCD churn the engine hard enough
 * to expose it). Recursive — scroll_now() is called both from the worker
